...
output_directory = r"/CCAS/groups/engstromgrp/paulina/features-analysis/outputs/"
text_file = "McGrath3_bands.txt"
...
def write_outputs(scales):
    ...
    for scale in scales:
        ...
        for output in outputs[feature]:
            ...
            output_tif = os.path.join(r"/CCAS/groups/engstromgrp/paulina/features-analysis/outputs/band/", feature, feature + "_sc" + str(scale) + "_" + output + ".tif")
            
            # Create the relative path for the clipped tif
            # Need to create a folder called /outputs/clipped with each feature named
           # output_clipped_tif = os.path.join(r"/CCAS/groups/engstromgrp/paulina/features-analysis/outputs/band/final/", feature, feature + "_sc" + str(scale) + "_" + output + ".tif")
            ...
            with open(os.path.join(output_directory, text_file), "a+") as file:
                ...
                #file.write("gdal_calc.py -A {} -B {} --outfile={} --NoDataValue=0 --extent=intersect --calc=\'A*(B>0)'\n\n".format (output_tif, clipraster, output_clipped_tif))
...
for feature in spfeas:
    ...
    folder = os.path.join(output_directory, "features", """McGrath3_""" + feature)
